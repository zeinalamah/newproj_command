    ...
    with requests.Session() as session:
        ...
        for attempt in range(10):
            try:
                ...
                response.raise_for_status();return response.json(),{'year':year,'product':product,'status':'ok','bytes':len(response.content),'attempts':attempt+1,'url':url}
...
def main():
    ...
    for year,product in itertools.product(YEARS,PRODUCTS):
        ...
        pivot=pivot.rename(columns={'VALUE_IN_EUROS':'trade_value_eur','QUANTITY_IN_KG':'quantity_kg','time':'month'})
        for col in ['trade_value_eur','quantity_kg']:
    ...
    coverage=data.groupby(['hs6',data.month.dt.year.rename('year')],as_index=False).agg(rows=('reporter','size'),reporters=('reporter','nunique'),partners=('partner','nunique'),quantity_kg=('quantity_kg','sum'),trade_value_eur=('trade_value_eur','sum'))
    coverage.to_csv(OUT/'coverage.csv',index=False)
    failed=[x for x in logs if x['status']!='ok' or x.get('parse_error')]
    summary={'rows':len(data),'first_month':str(data.month.min().date()),'latest_month':str(data.month.max().date()),'reporters':data.reporter.nunique(),'partners':data.partner.nunique(),'products':data.hs6.nunique(),'failed_requests':failed}
    ...
    if failed:raise RuntimeError('Some product-year requests failed')
