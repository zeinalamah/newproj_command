...
def main():
 ...
 with zipfile.ZipFile(ZIP) as z:
  names=z.namelist(); country=next(n for n in names if re.search(r"country.*codes.*\.csv$",Path(n).name,re.I)); product=next((n for n in names if re.search(r"product.*codes.*HS92.*\.csv$",Path(n).name,re.I)),None)
  (OUT/"zip_member_names.txt").write_text("\n".join(names),encoding="utf-8")
  ...
  annual=sorted(n for n in names if re.search(r"BACI_HS92_Y\d{4}_V\d{6}\.csv$",Path(n).name,re.I))
  if not annual:raise RuntimeError("No annual BACI members; see zip_member_names.txt")
 ...
 ZIP.unlink(missing_ok=True)
 if not selected:raise RuntimeError("Annual files were read but selected product rows were empty")
 ...
 cc.columns=[str(c).strip().lower() for c in cc.columns]; code=next(c for c in ["country_code","i","iso_num","iso3n"] if c in cc); iso=next(c for c in ["country_iso3","iso_3digit_alpha","iso3","iso3c"] if c in cc); name=next(c for c in ["country_name_full","country_name","name"] if c in cc); m=cc[[code,iso,name]].drop_duplicates(); m[code]=m[code].astype(str)
